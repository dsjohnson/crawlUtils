...
#' @export
#' @import sf
#' @importFrom terra ext crds crs ncell rast values
...
cu_sf_kde <- function (x, ref, ess=NULL, norm_kern=TRUE, mask = FALSE, bw = NULL)
{
  ...
  if (is.null(bw)) {
    h <- bw_silver(st_coordinates(x), ess)
    # message("Using ", round(bw[1], 3), ", ", round(bw[2], 3), " for bandwidth", "\n")
  }
  else {
    if(length(bw)==1) h <- c(bw, bw)
    if(length(bw)==2) h <- bw
  }
  # browser()
  ...
  k <- dnorm(outer(xy_ref[,1], xy_pts[,1], "-")/h[1]) * dnorm(outer(xy_ref[,2], xy_pts[,2], "-")/h[2])
